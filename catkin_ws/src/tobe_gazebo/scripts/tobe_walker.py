...
class WalkFunc:
	...
	if ((mu>K_mu0) and (mu<K_mu1)): # during right swing phase
	    if mu<K_mu2: # during leg lifting
	    	eta_R=math.sin(0.5*math.pi*(mu-K_mu0)/(K_mu2-K_mu0))*(K8+K9*max(abs(A)))
	    else: # during leg lowering
	    	eta_R=math.sin(0.5*math.pi*(1+(mu-K_mu2)/(K_mu1-K_mu2)))*(K8+K9*max(abs(A)))
	else: # during right stance phase
	    if mu>=K_mu1:
		nu_R1=-math.pi+math.pi*(mu-K_mu1)/(2*math.pi+K_mu0-K_mu1)
	    else:
		nu_R1=-math.pi+math.pi*(mu-K_mu1+2*math.pi)/(2*math.pi+K_mu0-K_mu1)
	...
	if ((mu>K_mu0-math.pi) and (mu<K_mu1-math.pi)): # during left swing phase
	    if mu<K_mu2-math.pi: # during leg lifting
	    	eta_L=math.sin(0.5*math.pi*(mu-K_mu0+math.pi)/(K_mu2-K_mu0))*(K8+K9*max(abs(A)))
	    else: # during leg lowering
	    	eta_L=math.sin(0.5*math.pi*(1+(mu-K_mu2+math.pi)/(K_mu1-K_mu2)))*(K8+K9*max(abs(A)))
	else: # during right stance phase
	    if mu>=K_mu1-math.pi:
		nu_L1=-math.pi+math.pi*(mu-K_mu1+math.pi)/(2*math.pi+K_mu0-K_mu1)
	    else:
		nu_L1=-math.pi+math.pi*(mu-K_mu1+3*math.pi)/(2*math.pi+K_mu0-K_mu1)
	...
	if ((mu>=K_mu0-math.pi) and (mu<K_mu1-math.pi)): # left swing phase
	    hip_R=prestance+stmax*math.sin(math.pi*(mu-K_mu0+math.pi)/(K_mu1-K_mu0))
	    hip_L=preswing+swmax*math.sin(math.pi*(mu-K_mu0+math.pi)/(K_mu1-K_mu0))
	    ank_R=prepro-pron*math.sin(math.pi*(mu-K_mu0+math.pi)/(K_mu1-K_mu0))
	    ank_L=presup+sup*math.sin(math.pi*(mu-K_mu0+math.pi)/(K_mu1-K_mu0))
	else:
	    if ((mu>=K_mu1-math.pi) and (mu<K_mu0)): # double support leading to right swing
	        hip_R=prestance+(-preswing-prestance)*math.sin(0.5*math.pi*(mu-K_mu1+math.pi)/(K_mu0-K_mu1+math.pi))
	        hip_L=preswing+(-prestance-preswing)*math.sin(0.5*math.pi*(mu-K_mu1+math.pi)/(K_mu0-K_mu1+math.pi))
	        ank_R=prepro+(presup-prepro)*(mu-K_mu1+math.pi)/(K_mu0-K_mu1+math.pi)
	        ank_L=presup+(prepro-presup)*(mu-K_mu1+math.pi)/(K_mu0-K_mu1+math.pi)
	    else:
		if ((mu>=K_mu0) and (mu<K_mu1)): # right swing phase
	    	    hip_R=-preswing-swmax*math.sin(math.pi*(mu-K_mu0)/(K_mu1-K_mu0))
	    	    hip_L=-prestance-stmax*math.sin(math.pi*(mu-K_mu0)/(K_mu1-K_mu0))
	    	    ank_R=presup+sup*math.sin(math.pi*(mu-K_mu0)/(K_mu1-K_mu0))
	    	    ank_L=prepro-pron*math.sin(math.pi*(mu-K_mu0)/(K_mu1-K_mu0))
		else:
		    if mu>=K_mu1: # double support leading to left swing, part 1
	        	hip_R=-preswing+(preswing+prestance)*math.sin(0.5*math.pi*(mu-K_mu1)/(K_mu0-K_mu1+math.pi))
	        	hip_L=-prestance+(prestance+preswing)*math.sin(0.5*math.pi*(mu-K_mu1)/(K_mu0-K_mu1+math.pi))
	        	ank_R=presup+(prepro-presup)*(mu-K_mu1)/(K_mu0-K_mu1+math.pi)
	        	ank_L=prepro+(presup-prepro)*(mu-K_mu1)/(K_mu0-K_mu1+math.pi)
		    else: # double support leading to left swing, part 2 (mu < K_mu0 - pi)
	        	hip_R=-preswing+(preswing+prestance)*math.sin(0.5*math.pi*(mu+2*math.pi-K_mu1)/(K_mu0-K_mu1+math.pi))
	        	hip_L=-prestance+(prestance+preswing)*math.sin(0.5*math.pi*(mu+2*math.pi-K_mu1)/(K_mu0-K_mu1+math.pi))
	        	ank_R=presup+(prepro-presup)*(mu+2*math.pi-K_mu1)/(K_mu0-K_mu1+math.pi)
	        	ank_L=prepro+(presup-prepro)*(mu+2*math.pi-K_mu1)/(K_mu0-K_mu1+math.pi)
	...
	S=[Sx,Sy,Sz]	# setting Sy,Sz to zero limits stepping to forward/backward only
	K20=3.5
	K21=0.2
	K22=0.2
	K23=0.2
	...
	A=(self.A+dA) #At+0* to use the algorithm as originally designed, remove 'At' and and the zero multiplier
	self.A=A
	return A
	...
	Rlegpitch=math.cos(-Rlegyaw)*phi_Rleg[1]-math.sin(-Rlegyaw)*phi_Rleg[0]
	Rlegroll=math.sin(-Rlegyaw)*phi_Rleg[1]+math.cos(-Rlegyaw)*phi_Rleg[0]
	Llegpitch=math.cos(-Llegyaw)*phi_Lleg[1]-math.sin(-Llegyaw)*phi_Lleg[0]
	Llegroll=math.sin(-Llegyaw)*phi_Lleg[1]+math.cos(-Llegyaw)*phi_Lleg[0]
	...
	f=[-0.2,-1.1932,-1.7264,0.4132,-0.15,0,0,0.15,-0.4132,1.7264,1.1932,0.2,-0.3927,-0.3491,-0.5236,0.3927,-0.3491,0.5236]
...
class Walker:
	...
	n=50
	    ...
	    if self.T<=0:	# if this value reaches zero (the foot should be touching surface)
	        self.T=self.Tinit      
